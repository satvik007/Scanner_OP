...
void writeImage (gray **gimg, Img* img) {
  cv::Mat dst (cv::Size(img->width, img->height), CV_8UC1);
  for (int i = 0; i < img -> height; i++) {
    for (int j = 0; j < img -> width; j++) {
      dst.at<uint8_t>(i, j) = gimg[i][j];
    }
  }
  cv::imwrite ("final2.jpg", dst);
}
...
int not_main( int argc, char *argv[] ) {
  ...
  char *optstr = (char*)"u:d:t:w:k:K:s:S:AM:e:P:Nar:R:x:y:pl:V:hv";
