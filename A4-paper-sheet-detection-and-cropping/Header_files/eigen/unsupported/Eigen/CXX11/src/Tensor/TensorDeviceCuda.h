...
struct GpuDevice {
  ...
  inline bool ok() const {
    ...
    return (error == cudaMySuccess) || (error == cudaErrorNotReady);
#else
    return false;
#endif
  }
...
#define LAUNCH_CUDA_KERNEL(kernel, gridsize, blocksize, sharedmem, device, ...)             \
  ...
  assert(cudaGetLastError() == cudaMySuccess);
