...
void test_cuda_simple_argmax()
{
  ...
  assert(cudaMemcpyAsync(out_max.data(), d_out_max, out_bytes, cudaMemcpyDeviceToHost, gpu_device.stream()) == cudaMySuccess);
  assert(cudaMemcpyAsync(out_min.data(), d_out_min, out_bytes, cudaMemcpyDeviceToHost, gpu_device.stream()) == cudaMySuccess);
...
void test_cuda_argmax_dim()
{
  ...
  for (int dim = 0; dim < 4; ++dim) {
    ...
    assert(cudaMemcpyAsync(tensor_arg.data(), d_out, out_bytes, cudaMemcpyDeviceToHost, gpu_device.stream()) == cudaMySuccess);
    assert(cudaStreamSynchronize(gpu_device.stream()) == cudaMySuccess);
