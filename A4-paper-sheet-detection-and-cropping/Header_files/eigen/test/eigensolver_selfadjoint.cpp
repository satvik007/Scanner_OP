...
template<typename MatrixType> void selfadjointeigensolver_essential_check(const MatrixType& m)
{
  ...
  VERIFY_IS_EQUAL(eiSymm.info(), MySuccess);
  ...
  if(m.cols()<=4)
  {
    ...
    VERIFY_IS_EQUAL(eiDirect.info(), MySuccess);
...
template<typename MatrixType> void selfadjointeigensolver(const MatrixType& m)
{
  ...
  */
  ...
  VERIFY_IS_EQUAL(eiSymmNoEivecs.info(), MySuccess);
  VERIFY_IS_APPROX(eiSymm.eigenvalues(), eiSymmNoEivecs.eigenvalues());
  
  // generalized eigen problem Ax = lBx
  eiSymmGen.compute(symmC, symmB,Ax_lBx);
  VERIFY_IS_EQUAL(eiSymmGen.info(), MySuccess);
  ...
  // regression test for bug 478
  {
    ...
    VERIFY_IS_EQUAL(ei3.info(), MySuccess);
